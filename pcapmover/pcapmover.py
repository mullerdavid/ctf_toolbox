...
async def add_elastic_task(path: Path):
    p = path.as_posix()
    pid = subprocess.Popen(f"(tshark -T ek -J 'http tcp udp ip ipv6' -x -r '{p}' | ./tshark-to-elastic '{elastic_host}/packets_template/_bulk' '{p}') &", shell=True, start_new_session=True)
